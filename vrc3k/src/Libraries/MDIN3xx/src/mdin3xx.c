...
static WORD fbADDR, GetDID, GetROW, GetIRQ = 0;
static WORD GetMEM = 0;
...
static ROMDATA MDIN_BWPOINT_COEF defMDINBWPoint[]	= {
	...
	{	{ 32,  64,  96, 128}, { 64,  96, 120, 136}	},	// type4 [in|out]
	{	{ 32,  48,  64,  80}, { 64,  96, 120, 186}	},	// type5 [in|out]	
};
...
static MDIN_ERROR_t MDIN3xx_SetSrcClockPath(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (pINFO->srcPATH) {
		...
		case PATH_MAIN_B_AUX_A: pSRC_a = &pINFO->stSRC_a; pSRC_b = &pINFO->stSRC_b;
							inA = MDIN_clk_ab_CLK_A; inB = MDIN_clk_ab_CLK_B; break;
	...
	// added on 21Sep2011 ===============================================================================//
	if (pINFO->srcPATH == PATH_MAIN_B_AUX_A) {
		if (MDINHIF_RegField(MDIN_HOST_ID, 0x03c, 11, 1, 1)) return MDIN_I2C_ERROR; // disable clk_m1
	}
	else {
		if (MDINHIF_RegField(MDIN_HOST_ID, 0x03c, 11, 1, 0)) return MDIN_I2C_ERROR; // enable clk_m1
	}
	//=============================================================================================//
...
static MDIN_ERROR_t MDIN3xx_SetSrcVideoPort(PMDIN_VIDEO_INFO pINFO, WORD nID)
{
	...
	switch (pSRC->frmt) {
		...
		case VIDSRC_960x480i60:		case VIDSRC_960x576i50:		//add lusifynn(Pinetron Co.,LTD)
		...
		case VIDSRC_1280x720p30:	case VIDSRC_1280x720p25:	
		...
		case VIDSRC_1920x1080p30:	case VIDSRC_1920x1080p25:
			pSRC->stATTB.attb |= (MDIN_QUALITY_HD|MDIN_VIDEO_SYSTEM); break;
		default:
	...
	mode = (pINFO->dacPATH==DAC_PATH_AUX_2HD)? 2 : 0;
...
static MDIN_ERROR_t MDIN3xx_SetOutVideoFrmt(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (pOUT->frmt) {
		...
		case VIDOUT_960x480i60:		case VIDOUT_960x576i50:		//add luisfynn(Pinetron Co.,LTD)
		...
		case VIDOUT_1280x720p30:	case VIDOUT_1280x720p25:	case VIDOUT_1280x720p24:
		...
		case VIDOUT_1920x1080p30:	case VIDOUT_1920x1080p25:	case VIDOUT_1920x1080p24:
			pOUT->stATTB.attb |= (MDIN_WIDE_RATIO|MDIN_QUALITY_HD|MDIN_VIDEO_SYSTEM); break;
	...
	switch (pOUT->mode) {
		case MDIN_OUT_RGB444_8:		mode = (2<<8)|(0<<5)|(1<<2)|0;
	...
	// digital_blank_data
	mode  = (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)? 0x1080 : 0x0000; // Y=16, CbCr=128
...
static MDIN_ERROR_t MDIN3xx_SetSrcVideoCSC(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (pSRC->stATTB.attb&MDIN_COLORSPACE_YUV) 
	{
		if (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)
			...
			pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscBypass_StdRange;	// YUV(HD or SD) to RGB(HD or SD)
	}		
	else if (pSRC->stATTB.attb&MDIN_QUALITY_HD) 
	{
		if (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)
			...
			pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_HD_StdRange;	// RGB(HD) to RGB(HD or SD)
	}
	else 
	{
		if (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)
			...
			pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_SD_StdRange;	// RGB(SD) to RGB(HD or SD)
	}
	...
	{
		if (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)
			pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_HD_FullRange;	// RGB(HD) to YUV(HD or SD) 
		else
			pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_HD_FullRange;	// RGB(HD) to RGB(HD or SD)
	}
	else 
	{
		if (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)
			pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_SD_FullRange;	// RGB(SD) to YUV(HD or SD) 
		else
			pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_SD_FullRange;	// RGB(SD) to RGB(HD or SD)
	}
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetSrcVideoCSC_test(BYTE index)
{
	MDIN_CSCCTRL_INFO	stCSC, *pCSC;

	switch(index)
	{
		case 0 :pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscBypass_StdRange;break;		// YUV(HD or SD) to YUV(HD or SD)
		case 1 :pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_HD_FullRange;break;	// RGB(HD) to YUV(HD or SD) 
		default : pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_SD_FullRange;break;	// RGB(SD) to RGB(HD or SD)
	}
	memcpy(&stCSC, (PBYTE)pCSC, sizeof(MDIN_CSCCTRL_INFO));
...
static MDIN_ERROR_t MDIN3xx_SetOutVideoSYNC(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (pINFO->dacPATH==DAC_PATH_AUX_4CH) {		// for 4-CH input mode
		...
		/*
				if (pAUX->frmt==VIDOUT_1920x1080p60 || pAUX->frmt==VIDOUT_1920x1080p59 || pAUX->frmt==VIDOUT_1920x1080p30) stSYNC.totHS = 1082;
				if (pAUX->frmt==VIDOUT_1920x1080p50 || pAUX->frmt==VIDOUT_1920x1080p25 || pAUX->frmt==VIDOUT_1920x1080p24) stSYNC.totHS = 1090;
				...
				if (pAUX->frmt==VIDOUT_1920x1080p50 || pAUX->frmt==VIDOUT_1920x1080p25 || pAUX->frmt==VIDOUT_1920x1080p24) stSYNC.totHS = 1090; 
		*/
		/* // deleted on 12Mar2014
		switch (pOUT->frmt) {
			case VIDOUT_720x480p60:		stSYNC.totHS = 1082; break;
			case VIDOUT_720x576p50: 	stSYNC.totHS = 1090; break;
			case VIDOUT_960x480p60:		stSYNC.totHS = 1282; break;
			case VIDOUT_960x576p50: 	stSYNC.totHS = 1290; break;
			case VIDOUT_1280x480p60:	stSYNC.totHS = 1782; break;
			case VIDOUT_1280x576p50: 	stSYNC.totHS = 1790; break;
			case VIDOUT_1440x480p60:	stSYNC.totHS = 1782; break;
			case VIDOUT_1440x576p50: 	stSYNC.totHS = 1790; break;
		}
		*/
	}
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x083, 6, 1, 1)) return MDIN_I2C_ERROR;	// fix main_skip_dis // deleted on 16Aug2012
	...
	// bg_color_y, cbcr
	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x09a, 0x0010)) return MDIN_I2C_ERROR;	// fix black
...
//		memcpy(buff, ((PWORD)pCoef)+i*2, 4);	buff[4] = MAKEWORD(nID,(0x80|i));
	...
	for (i=0; i<sizeof(MDIN_MFCFILT_COEF)/8; i++) { 		// for 2-word write
		memcpy(buff, ((PWORD)pCoef)+i*4, 8);	buff[4] = MAKEWORD(nID,(0x80|i));
...
//	WORD mode = (pMFC->stFFC.sh>=pMFC->stDST.h*2)? (3<<2)|3 : (2<<2)|2;
	WORD mode = (pMFC->stFFC.sh*2>=pMFC->stDST.h*3)? (3<<2)|3 : (2<<2)|2;	// set VY, VC mfc filter on 16Aug2012

	// mfc_control1 - HY(low), HC(low), VY(dual), VC(low)
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x100, 8, 8, (2<<6)|(2<<4)|mode)) return MDIN_I2C_ERROR;  //jungwan,20140401
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x100, 8, 8, (2<<6)|(3<<4)|(3<<2)|3)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x100, 8, 8, (3<<6)|(3<<4)|(3<<2)|3)) return MDIN_I2C_ERROR; //20140424 jungwan
...
//	if (pSRC->stATTB.attb&MDIN_SCANTYPE_PROG) pSRC->stATTB.attb &= ~MDIN_PRECISION_8;	// set 10-bit processing, delete on 20Mar2012
	...
	if ((pSRC->stATTB.attb&MDIN_SCANTYPE_PROG)&&
	   (pMFC->stMEM.h==1080&&pMFC->stDST.h==576)) pSRC->stATTB.attb |= MDIN_PRECISION_8; // prepare for timing problem when 3dnr on, 20Mar2012

	if (pSRC->frmt==VIDSRC_1920x1080i50||pSRC->frmt==VIDSRC_1920x1080i60) pSRC->stATTB.attb |= MDIN_PRECISION_8; // prepare for timing problem when 3dnr on, 16Aug2012
...
static MDIN_ERROR_t MDIN3xx_SetIPCCtrlFlags(PMDIN_VIDEO_INFO pINFO)
{
	...
	if ((pSRC->frmt==VIDSRC_720x576i50) || (pSRC->frmt==VIDSRC_1920x1080i50)|| (pSRC->frmt==VIDSRC_960x576i50))
...
//	if (pMFC->stSRC.h>=pMFC->stDST.h*2)
	// set p5-scale flag, if scale ratio < 0.5
	if ((pMFC->stSRC.h) > (pMFC->stDST.h*2))		// modified on 20Mar2012
...
static MDIN_ERROR_t MDIN3xx_SetDeinterCtrl(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x208, 11, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x208, 10, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;
...
//	}
	...
	if ((pIPC->attb & MDIN_DEINT_HD_1080i) &&
		(pOUT->stATTB.attb&MDIN_SCANTYPE_PROG)&&(pMFC->stDST.h<=540)) // c_dein_mode is intra only mode, modified on 20Mar2012
	{ 
		if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x249, 12, 2, 2)) return MDIN_I2C_ERROR; // c intra
		if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x252,  0, 2, 1)) return MDIN_I2C_ERROR;
	}
	else 
	{
		if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x249, 12, 2, 1)) return MDIN_I2C_ERROR; // c adaptive
		if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x252,  0, 2, 3)) return MDIN_I2C_ERROR;
	}

	// progressive ipc mode - 20Mar2012
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x100,  3, 1, (pIPC->attb&MDIN_DEINT_PROG_IPC)? 1 : 0)) return MDIN_I2C_ERROR; // mfc_interlaced_input
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x208, 15, 1, (pIPC->attb&MDIN_DEINT_PROG_IPC)? 1 : 0)) return MDIN_I2C_ERROR; // enable p-ipc mode
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x083,  5, 1, (pIPC->attb&MDIN_DEINT_PROG_IPC)? 1 : 0)) return MDIN_I2C_ERROR; // main_repeat_dis

	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x083,  6, 1, (pIPC->attb&MDIN_DEINT_FRC_DOWN)? 0 : 1)) return MDIN_I2C_ERROR; // main_skip_dis, added on 28Dec2012
	...
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x040, 4, 1, MBIT(pIPC->fine,MDIN_PROCESS_444))) return MDIN_I2C_ERROR;
...
static MDIN_ERROR_t MDIN3xx_SetMemoryMap(PMDIN_VIDEO_INFO pINFO, BYTE nID, BYTE num, WORD addr)
{
	...
	bpp = ((nID%4)==2||((pSRC->stATTB.attb) & MDIN_PRECISION_8))?  8 : 10;	// bit per pixel
	bpc = ((nID%4)==2||((pSRC->stATTB.attb) & MDIN_PRECISION_8))? 64 : 60;	// bit per column
	...
	// nID: the number of memory block 
	// num: a number of frame memory
	// addr: start row addr point for appending
	// fbADDR: end row addr point
...
static MDIN_ERROR_t MDIN3xx_SetFrameBuffer(PMDIN_VIDEO_INFO pINFO)
{
	...
	// added on 20Mar2012
	if (pIPC->attb&MDIN_DEINT_PROG_IPC) {
		fmap = pMAP->frmt;			// save pMAP
		pMAP->frmt = MDIN_MAP_AUX_ON_NR_OFF;
		pSRC->stATTB.attb |=  MDIN_SCANTYPE_PROG;
	}
...
//	pMAP->Y_m += pMAP->Ynr;					// fbuf0y
	if (MDIN3xx_SetMemoryMap(pINFO, 0+0, pMAP->Y_m+pMAP->Ynr, addr)) return MDIN_I2C_ERROR;
	...
	if (pIPC->attb&MDIN_DEINT_PROG_IPC)	pMAP->frmt = fmap;	// restore pMAP // added on 20Mar2012
...
static MDIN_ERROR_t MDIN3xx_Set4CHProcess(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (pINFO->st4CH_x.view) {
		...
		case MDIN_4CHVIEW_CH12: 
		case MDIN_4CHVIEW_CH12_V:
			rVal = nA1|(nA2<<2)|(nB1<<4)|(nB2<<6); break;
		case MDIN_4CHVIEW_CH34: 
		case MDIN_4CHVIEW_CH34_V:
			rVal = nB1|(nB2<<2)|(nB1<<4)|(nB2<<6); break;
		default:				rVal = nA1|(nA2<<2)|(nB1<<4)|(nB2<<6); break;
	}
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x14a, 0, 8, rVal)) return MDIN_I2C_ERROR;
...
//	if (MDIN3xx_SetDeinterCtrl(pINFO)) return MDIN_I2C_ERROR;		// set deinterlace control	// move to here on 16Aug2012
...
static MDIN_ERROR_t MDIN3xx_SetZoomProcess(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDIN3xx_SetDeinterCtrl(pINFO)) return MDIN_I2C_ERROR;   //jung
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetCDeint(BYTE flag)
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x249, 12, 2, (flag==ON)?3:1)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetColorTonFilterCoef(PMDIN_COLORTON_COEF pCoef)
{
	if (pCoef==NULL) pCoef = (PMDIN_COLORTON_COEF)MDIN_ColorTonFilter_Default;
	if (MDINHIF_MultiWrite(MDIN_LOCAL_ID, 0x3f0, (PBYTE)pCoef, 10)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}

//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_EnableColorTonFilter(BOOL OnOff)
{
...
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x208, 10, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_EnableMirrorH(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	UnitSize = (pSRC->stATTB.attb&MDIN_PRECISION_8)? 256 : 192;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x01c, 0, 11, (OnOff)? (pMFC->stFFC.dw/UnitSize)*UnitSize : 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x01c, 13, 1, RBIT(OnOff,1))) return MDIN_I2C_ERROR;	
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x01c, 15, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;

//	BuffSize = ROUNDUP(pMFC->stFFC.dw, UnitSize)*UnitSize;
	...
	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x034, (OnOff)? RemainSize : 0)) return MDIN_I2C_ERROR;
...
static MDIN_ERROR_t MDIN3xx_SetSrcVideoPattern(PMDIN_VIDEO_INFO pINFO, MDIN_IN_TEST_t mode)
{
	...
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x042, 6, 1, 1)) return MDIN_I2C_ERROR;
...
//	if (MDINHIF_RegField(MDIN_HOST_ID, 0x046,  3,  2,  2)) return MDIN_I2C_ERROR;	// set clk_aux_out as axclk
	...
	if (MDINHIF_RegField(MDIN_HOST_ID, 0x042,  2,  2,  3)) return MDIN_I2C_ERROR;	// set vclk_src as axclk_src
...
MDIN_ERROR_t MDIN3xx_SetOut4CH_OutVsync_Half(BOOL OnOff)	// 24Aug2011
{
	WORD rVal = 0;
 	...
 	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x249, 12,  2, (OnOff)? 1:2)) return MDIN_I2C_ERROR; // set c intra mode (to reduce b/w)
 	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x01e,  6,  1, (OnOff)? 1:0)) return MDIN_I2C_ERROR; // mch_wr_field_en
 	...
 	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x088, (OnOff)? rVal*2:rVal)) return MDIN_I2C_ERROR; // half frequency (vsync: 60 to 30Hz, 50 to 25Hz) for codec
#else
 	if (MDINHIF_RegRead(MDIN_HOST_ID, 0x030, &rVal)) return MDIN_I2C_ERROR;
 	if (MDINHIF_RegWrite(MDIN_HOST_ID, 0x030, (OnOff)? rVal*2:rVal)) return MDIN_I2C_ERROR; // half frequency (vsync: 60 to 30Hz, 50 to 25Hz) for codec
#endif
