...
static MDIN_ERROR_t MDINAUX_SetOutVideoFrmt(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (pOUT->mode) {
		case MDIN_OUT_RGB444_8:		mode = (0<<11)|(0<<7)|(1<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_444|MDIN_COLORSPACE_RGB); break;
		case MDIN_OUT_YUV444_8:		mode = (1<<11)|(0<<7)|(0<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_444|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_EMB422_8:		mode = (1<<11)|(2<<7)|(0<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_SEP422_8:		mode = (1<<11)|(0<<7)|(0<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_EMB422_10:	mode = (1<<11)|(2<<7)|(0<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_10|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_SEP422_10:	mode = (1<<11)|(0<<7)|(0<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_10|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_MUX656_8:		mode = (1<<11)|(3<<7)|(0<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_MUX656_10:	mode = (1<<11)|(3<<7)|(0<<3);
			pOUT->stATTB.attb |= (MDIN_PRECISION_10|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
	}
	// for 4-CH input mode, 2-HD input mode
	if (pINFO->dacPATH==DAC_PATH_AUX_4CH||pINFO->dacPATH==DAC_PATH_AUX_2HD) mode |= (1<<8); // aux_digital_sync_en
...
static MDIN_ERROR_t MDINAUX_SetSrcVideoCSC(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (pSRC->stATTB.attb&MDIN_COLORSPACE_YUV) {
		...
		if (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)
			 pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_HD_FullRange;	// RGB(HD) to YUV(HD or SD)	// modified on 16Aug2012
		else pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_HD_FullRange;	// RGB(HD) to RGB(HD or SD)
	}
	else {
		if (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)
			 pCSC = (PMDIN_CSCCTRL_INFO)&MDIN_CscRGBtoYUV_SD_FullRange;	// RGB(SD) to YUV(HD or SD) // modified on 16Aug2012
...
/*		case MDIN_4CHVIEW_CH13:
		case MDIN_4CHVIEW_CH14: case MDIN_4CHVIEW_CH23: 
		case MDIN_4CHVIEW_CH24:
*/		case MDIN_4CHVIEW_CH34:
				 pMFC->st4CH.w = pMFC->stDST.w/2; pMFC->st4CH.h = pMFC->stDST.h/1; break;

		// added on 04Mar2014
		case MDIN_4CHVIEW_CH12_V:
/*		case MDIN_4CHVIEW_CH13_V:
		case MDIN_4CHVIEW_CH14_V: case MDIN_4CHVIEW_CH23_V: 
		case MDIN_4CHVIEW_CH24_V:
*/		case MDIN_4CHVIEW_CH34_V:
				pMFC->st4CH.w = pMFC->stDST.w/1; pMFC->st4CH.h = pMFC->stDST.h/2; break;
...
//	if ((pSRC->stATTB.attb&MDIN_SCANTYPE_PROG)&&(pMFC->stCUT.h>pMFC->stDST.h)&&
	if ((pSRC->stATTB.attb&MDIN_SCANTYPE_PROG)&&(pMFC->stCUT.h>(pMFC->stDST.h*2))&&		// modified on 29Mar2012
...
//--------------------------------------------------------------------------------------------------------------------------
static MDIN_ERROR_t MDINAUX_SetFFCNRProcess(PMDIN_VIDEO_INFO pINFO)	// added on 16Aug2012
{
	BYTE ratio=0; PMDIN_AUXFILT_COEF pCoef;

	if (pINFO->srcPATH==PATH_MAIN_A_AUX_M || pINFO->srcPATH==PATH_MAIN_B_AUX_M) {
		ratio=(BYTE)((FLOAT)pINFO->stOUT_m.stATTB.Hact/(FLOAT)pINFO->stOUT_x.stATTB.Hact*10);
	}
	else if (pINFO->srcPATH==PATH_MAIN_A_AUX_A || pINFO->srcPATH==PATH_MAIN_B_AUX_A) {
		ratio=(BYTE)((FLOAT)pINFO->stSRC_a.stATTB.Hact/(FLOAT)pINFO->stOUT_x.stATTB.Hact*10);
	}
	else if (pINFO->srcPATH==PATH_MAIN_B_AUX_B || pINFO->srcPATH==PATH_MAIN_A_AUX_B) {
		ratio=(BYTE)((FLOAT)pINFO->stSRC_b.stATTB.Hact/(FLOAT)pINFO->stOUT_x.stATTB.Hact*10);
	}
//	UARTprintf("ratio = %d\n", ratio);
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDINAUX_SetOutTestPattern(MDIN_OUT_TEST_t mode)
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x14c, 10, 4, mode)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
// modified on 29Feb2012
	...
	if (MDINHIF_RegField(MDIN_HOST_ID, 0x036,  0, 12,  1)) return MDIN_I2C_ERROR; 	// f
		...
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x088, 1286)) return MDIN_I2C_ERROR;	// main output htotal for 960H NTSC input
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x08a, 1050)) return MDIN_I2C_ERROR; 	// hactive end
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x176, 1235)) return MDIN_I2C_ERROR;	// aux output vtotal for 1900x1200@RB 60Hz(30Hz) output
	#endif
	#if defined(PAL_4D1_IN) 	// for PAL input
 		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x088, 1296)) return MDIN_I2C_ERROR;	// main output htotal for 960H PAL input
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x08a, 1060)) return MDIN_I2C_ERROR; 	// hactive end
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x176, 1482)) return MDIN_I2C_ERROR;	// aux output vtotal for 1900x1200@RB 50Hz(25Hz) output
	#endif
	...
	#if defined(NTSC_4D1_IN)	// for NTSC input
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x088, 918)) return MDIN_I2C_ERROR;		// main output htotal for NTSC input
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x176, 1250)) return MDIN_I2C_ERROR;		// aux output vtotal for 1600x1200@60Hz(30Hz) output
	#endif
	#if defined(PAL_4D1_IN) 	// for PAL input
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x088, 926)) return MDIN_I2C_ERROR;		// main output htotal for PAL input
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x176, 1500)) return MDIN_I2C_ERROR;		// aux output vtotal for 1600x1200@50Hz(25Hz) output
	#endif
#endif
...
MDIN_ERROR_t MDINAUX_SetOut4CH_OutBT656(void) // 01Aug2011
{
	...
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x142,  3,  1,  1)) return MDIN_I2C_ERROR; // aux front v down-scaler bypass
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x142,  4,  1,  1)) return MDIN_I2C_ERROR; // aux front h down-scaler bypass
	...
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x14a, 12,  4,  0)) return MDIN_I2C_ERROR; // aux_mwin_v, aux_mwin_h (no mwin)
