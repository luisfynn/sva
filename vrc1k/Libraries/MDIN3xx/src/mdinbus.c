...
#include	"mdinbus.h"		// cpu dependent for bus & dma operation

static WORD	col_old = 0xffff;	//by hungry 2013.06.03

#define EPI0_BASE 0x400D0000
//#define		MDIN380_BASE_ADDR		((VPWORD)0xa0000000)
//--------------------------------------------------------------------------------------------------------------------------
#if 0
MDIN_ERROR_t MDINBUS_SetDMA(DWORD rAddr, WORD bytes, BOOL dir)	//by hungry 2013.05.13 for spi-rom
{
	...
	MDINHIF_RegWrite(MDIN_HOST_ID, 0x006, row);		// set DMA row+bank address
	if (col != col_old)	{MDINHIF_RegWrite(MDIN_HOST_ID, 0x008, col);	col_old = col;}	//by hungry 2013.06.03

	if (ctl == 0x0a)	MDINHIF_RegWrite(MDIN_HOST_ID, 0x014, num);		// set DMA block counter	//by hungry 2013.06.03
	else				MDINHIF_RegWrite(MDIN_HOST_ID, 0x00a, num);		// set DMA counter (read)

	MDINHIF_RegWrite(MDIN_HOST_ID, 0x00c, ctl);		// set DMA block start
...
//--------------------------------------------------------------------------------------------------------------------------
static void EPIDividerSet(DWORD ulBase, DWORD ulDivider)
{

}

DWORD GET_MDIN_DREQ( void )
{
	return 0;
}
...
static void MHDMI_BUSRead(WORD rAddr, PBYTE pBuff, WORD bytes)
{
	...
	for (i=0; i<bytes/2; i++) {
		...
		wctrl = 0x0002;		MDINBUS_Write(MDIN_HOST_ID, 0x027*2, (PBYTE)&wctrl, 2);
		err = MHDMI_GetWriteDone(); if (err) break;	// check done flag
		MDINBUS_Read(MDIN_HOST_ID, 0x026*2, (PBYTE)&wdata, 2);	((PWORD)pBuff)[i]  = (wdata&0xFF)<<0;

		waddr = waddr+1;	MDINBUS_Write(MDIN_HOST_ID, 0x025*2, (PBYTE)&waddr, 2);
		wctrl = 0x0003;		MDINBUS_Write(MDIN_HOST_ID, 0x027*2, (PBYTE)&wctrl, 2);
		wctrl = 0x0002;		MDINBUS_Write(MDIN_HOST_ID, 0x027*2, (PBYTE)&wctrl, 2);
		err = MHDMI_GetWriteDone(); if (err) break;	// check done flag
		MDINBUS_Read(MDIN_HOST_ID, 0x026*2, (PBYTE)&wdata, 2);	((PWORD)pBuff)[i] |= (wdata&0xFF)<<8;
	}
#endif
