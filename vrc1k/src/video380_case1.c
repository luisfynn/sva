...
///////////////////////////////////////////////////////////////////////
	...
	MDIN3xx_SetDIGOutMapMode(MDIN_DIG_OUT_X_MAP9);	// main -> digital out enable
	...
	// set video path
	//stVideo.srcPATH = PATH_MAIN_B_AUX_M;	// set main is B, aux is MAIN 
	stVideo.srcPATH = PATH_MAIN_B_AUX_B;	// set main is B, aux is MAIN 
...
#if 0
	if ((resolution==VIDSRC_1280x720p30)||(resolution==VIDSRC_1280x720p25))
	{
		stVideo.stSRC_m.stATTB.attb = MDIN_SCANTYPE_PROG;
	}
	else
	...
	stVideo.stSRC_b.frmt = VIDSRC_1280x720p30; 		//test lusifynn 141024
...
//	stVideo.stSRC_b.fine = MDIN_FIELDID_INPUT | MDIN_LOW_IS_TOPFLD;
	...
	if(vformat == VID_VENC_NTSC_M)
		...
		stVideo.stOUT_x.frmt = VIDOUT_1280x720p25;
...
//	stVideo.st4CH_x.order = MDIN_4CHID_B1B2A1A2; // set CH-ID mapping
...
//	MDIN3xx_SetDelayVCLK_OUT(7);	
	...
	MDINHIF_RegField(MDIN_LOCAL_ID, 0x145, 0, 3, 1); // aux pixel repetition
	MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x168, 0x500);               // 1280
	MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x16C, 0x500);              // 1280
	MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x142, 0xca7d);             
}
